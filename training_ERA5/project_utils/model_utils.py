...
class MulticlassAUC(metrics.AUC):
    ...
    ----------
    pos_label : int
        Label orf the positive class (the one whose AUC is being computed).
...
#def build_model(lr = .001, conv_filters = 32, dense_neurons = 16, dense_layers = 1, 
#                activity_reg = 0.001, dropout_rate = 0.2, input_channels = 3):

def build_model(lr = .0004, conv_filters = 16, dense_neurons = 16, dense_layers = 1, 
                activity_reg = 0.001, dropout_rate = 0.2, input_channels = 3):
    model = models.Sequential() 
    model.add(layers.Input(shape=(161, 341, input_channels))) ## define input shape
...
def build_model_24layer(lr=0.0004, conv_filters=16, dense_neurons=32, dense_layers=2,
                activity_reg=0.001, dropout_rate=0.2, input_channels=3,
                input_height=161, input_width=341):

    model = models.Sequential()
    model.add(layers.Input(shape=(input_height, input_width, input_channels)))

    h, w = input_height, input_width  # Track dimensions

    # Add 10 Conv2D + ReLU + optional MaxPooling + Dropout layers (20 layers total)
    for i in range(10):
        model.add(layers.Conv2D(conv_filters, (3, 3), padding='same',
                                activity_regularizer=regularizers.l2(activity_reg)))
        model.add(layers.Activation('relu'))
        
        if i % 2 == 1 and h >= 2 and w >= 2:
            model.add(layers.MaxPooling2D((2, 2)))
            h //= 2
            w //= 2
    ...
    model.add(layers.Flatten())  # Layer 21

    # Add dense layers (layers 22, 23)
    for _ in range(dense_layers):
        model.add(layers.Dense(dense_neurons,
                               activity_regularizer=regularizers.l2(activity_reg)))
        model.add(layers.Activation('relu'))

    # Final classifier layer (layer 24)
...
# run only once after any change to above "build_model" function. like "shape=(17, 35 ". then comment out.
#model = build_model_24layer() # build your model
#model.save_weights('../processed_data/initial_weights_24layer.h5') # save the weights

model = build_model() # build your model
model.save_weights('../processed_data/initial_weights.h5') # save the weights

#Tuned_weights
#model = build_model()
#model.save_weights('../processed_data/initial_weights_lr001_convFil32.h5') # save the weights
